...
public:
  ...
  /*!
   * \brief Offset the sampled values by a constant
   *
   * Used after applying a jitter correction to update all tracked
   * jitter values so they reflect the new baseline. Only the collected
   * samples are shifted (the same range the statistics functions read), so
   * not-yet-written slots cannot skew later results as the window fills.
   *
